...
export class PublicationQueryController {
  ...
  @GrpcMethod(
    GRPC_SERVICES.PUBLICATION_QUERY_SERVICE,
    PUBLICATION_METHODS.GET_POST_NODE,
  )
  async getPostNode(
    data: GetSocialPostQueryDTO,
  ): Promise<GetPostNodeResponseDTO> {
  ...
  @GrpcMethod(
    GRPC_SERVICES.PUBLICATION_QUERY_SERVICE,
    PUBLICATION_METHODS.GET_USER_POSTS,
  )
  async getUserPosts(
    ...
    const { userId, pagination } =
      PublicationMapper.toGetUserPostsQueryParams(data);
