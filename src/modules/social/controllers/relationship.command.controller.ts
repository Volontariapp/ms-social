...
export class RelationshipCommandController {
  ...
  @GrpcMethod(
    ...
    RELATIONSHIP_METHODS.POST_FOLLOW_USER,
  )
  async postFollowUser(
    data: PostFollowUserCommandDTO,
  ): Promise<PostFollowUserResponseDTO> {
    this.logger.log(
      `gRPC: User ${data.followerId} following ${data.followedId}`,
    );
    const { followerId, followedId } =
      RelationshipMapper.toFollowUserParams(data);
  ...
  @GrpcMethod(
    GRPC_SERVICES.RELATIONSHIP_COMMAND_SERVICE,
    RELATIONSHIP_METHODS.DELETE_FOLLOW_USER,
  )
  async deleteFollowUser(
    data: DeleteFollowUserCommandDTO,
  ): Promise<DeleteFollowUserResponseDTO> {
    this.logger.log(
      `gRPC: User ${data.followerId} unfollowing ${data.followedId}`,
    );
    const { followerId, followedId } =
      RelationshipMapper.toUnfollowUserParams(data);
  ...
  @GrpcMethod(
    GRPC_SERVICES.RELATIONSHIP_COMMAND_SERVICE,
    RELATIONSHIP_METHODS.POST_BLOCK_USER,
  )
  async postBlockUser(
  ...
  @GrpcMethod(
    GRPC_SERVICES.RELATIONSHIP_COMMAND_SERVICE,
    RELATIONSHIP_METHODS.DELETE_BLOCK_USER,
  )
  async deleteBlockUser(
    data: DeleteBlockUserCommandDTO,
  ): Promise<DeleteBlockUserResponseDTO> {
    this.logger.log(
      `gRPC: User ${data.blockerId} unblocking ${data.blockedId}`,
    );
    const { blockerId, blockedId } =
      RelationshipMapper.toUnblockUserParams(data);
