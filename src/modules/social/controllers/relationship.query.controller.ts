...
export class RelationshipQueryController {
  ...
  @GrpcMethod(
    ...
    RELATIONSHIP_METHODS.GET_MY_FOLLOWS,
  )
  async getMyFollows(
  ...
  @GrpcMethod(
    GRPC_SERVICES.RELATIONSHIP_QUERY_SERVICE,
    RELATIONSHIP_METHODS.GET_MY_FOLLOWERS,
  )
  async getMyFollowers(
    ...
    const { userId, pagination } =
      RelationshipMapper.toGetFollowersParams(data);
    const paginationVO = pagination ?? new PaginationVO(1, 10);
  ...
  @GrpcMethod(
    GRPC_SERVICES.RELATIONSHIP_QUERY_SERVICE,
    RELATIONSHIP_METHODS.GET_MY_BLOCKS,
  )
  async getMyBlocks(
  ...
  @GrpcMethod(
    GRPC_SERVICES.RELATIONSHIP_QUERY_SERVICE,
    RELATIONSHIP_METHODS.GET_WHO_BLOCKED_ME,
  )
  async getWhoBlockedMe(
    ...
    const { userId, pagination } =
      RelationshipMapper.toGetWhoBlockedMeParams(data);
    const paginationVO = pagination ?? new PaginationVO(1, 10);
    const paginatedIds = await this.service.getWhoBlockedMe(
      userId,
      paginationVO,
    );
    return PaginatedIdsMapper.toPaginatedIdsResponseDTO(paginatedIds);
